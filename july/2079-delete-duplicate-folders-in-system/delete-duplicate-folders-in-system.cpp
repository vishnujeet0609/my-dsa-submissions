// Approach (Using trie)
// T.C : O(N * L * ClogC), N = total Paths, L = average length of each path, C
// is the average number of children per node S.C : ~O(N * L), we store all the
// paths in the trie, approximated value.
class Solution {
public:
    struct Node {
        string val;       // name
    ...
    string populateNodes(Node* root, unordered_map<string, int>& subFolderMap) {
        ...
        string completePath = ""; // this is the subfolder of current root which
                                  // we will form from children paths
    ...
    void construstResult(Node* root, vector<string>& path,
