// T.C : O(mlogm + m*log(n)) , where Let n = number of rooms, m =  number of
// meetings S.C : O(n)
class Solution {
public:
    ...
    int mostBooked(int n, vector<vector<int>>& meetings) {
        ...
        sort(begin(meetings),
             end(meetings)); // sort by starting time of meetings

        vector<int> roomsUsedCount(
            n, 0); // Each room is used 0 times in the beginning
        ...
        for (vector<int>& meet : meetings) {
            ...
            if (!unusedRooms.empty()) {
                ...
                roomsUsedCount[room]++;
            } else { // We don't have any room available now. Pick earliest end
                     // room
                int room = usedRooms.top().second;
